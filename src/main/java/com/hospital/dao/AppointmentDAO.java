...
public class AppointmentDAO {
    ...
    public boolean bookAppointment(Appointment appointment) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (appointment.getStatus() != null) {
                ...
                pstmt.setString(5, AppointmentStatus.SCHEDULED.name());
            }
    ...
    public boolean cancelAppointment(int appointmentId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            pstmt.setString(1, AppointmentStatus.CANCELLED.name());
    ...
    public boolean isDoctorAvailable(int doctorId, LocalDate date, LocalTime time) {
        String sql = "SELECT COUNT(*) FROM appointments WHERE doctor_id = ? AND appointment_date = ? " +
                "AND appointment_time = ? AND status != ?";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            pstmt.setString(4, AppointmentStatus.CANCELLED.name());
    ...
    private Appointment mapRowToAppointment(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            // Handle case where DB has a status that doesn't exist in Enum
            System.err.println("Unknown status in DB: " + rs.getString("status"));
            appointment.setStatus(AppointmentStatus.SCHEDULED);
        }
